...
type TourRecord = (typeof toursData.tours)[number];
...
function getEffectiveTime(tour: TourRecord) {
  if (!tour.date) return Number.POSITIVE_INFINITY;
...
export function getNextTour(now = new Date()) {
  ...
  return toursData.tours
    .map((tour, index) => ({ tour, index, effectiveTime: getEffectiveTime(tour) }))
    .filter(({ effectiveTime }) => effectiveTime >= today)
    .sort((a, b) => a.effectiveTime - b.effectiveTime || a.index - b.index)[0]?.tour;
}

export function getTourCity(tour: TourRecord) {
